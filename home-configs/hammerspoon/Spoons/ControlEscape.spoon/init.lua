...
function obj:init()
  ...
  self.controlKeyTimer = hs.timer.delayed.new(CANCEL_DELAY_SECONDS,
                                              function() self.sendEscape = false end)
  ...
  self.controlTap = hs.eventtap.new({hs.eventtap.event.types.flagsChanged}, function(event)
    ...
    if not self.lastModifiers['ctrl'] then
      ...
      if self.sendEscape then hs.eventtap.keyStroke({}, 'escape', 0) end
      self.lastModifiers = newModifiers
