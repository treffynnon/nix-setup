...
# Provides utilities for creating standardized user configurations and nix settings
{ lib, ... }:
let
  ...
  getSecret = secretName: defaultValue: 
    let envVar = builtins.getEnv "NIX_${lib.toUpper secretName}";
    in if envVar != "" then envVar else defaultValue;
    
  # Safe secret handling - never expose actual values in error messages
  safeGetSecret = secretName: defaultValue:
    let 
    ...
    in {
      ...
      source = if isFromEnv then "environment" else "default";
    };
...
in {
  ...
  # Helper to create a standardized user configuration
  mkUser = { username ? defaults.user.username, shell ? "fish", extraGroups ? [], extraConfig ? {} }:
    {
      name = username;
      value = {
        shell = shell;
        home = if builtins.elem builtins.currentSystem ["aarch64-darwin" "x86_64-darwin"] 
               then "/Users/${username}" 
               else "/home/${username}";
  ...
  mkUserWithSecrets = { username ? defaults.user.username, shell ? null, extraGroups ? [], extraConfig ? {} }:
    let
    ...
    in {
      name = username;
      value = {
        home = if defaults.platform.isDarwin 
               then "/Users/${username}" 
               else "/home/${username}";
        shell = shell;
        extraGroups = extraGroups;
        # Use secrets-aware SSH key
        openssh.authorizedKeys.keys = lib.optional (sshKey != null) sshKey;
      } // extraConfig;
    };

  # Helper to create a standardized home-manager user configuration
  mkHomeManagerUser = { stateVersion ? defaults.versions.homeManagerStateVersion, extraConfig ? {} }:
    {
      home.stateVersion = stateVersion;
    } // extraConfig;

  # Helper to create standardized nix configuration
  ...
  mkAutomatedPackageManagement = { enableAutoUpdate ? true, enableAutoGC ? true, enableFlakeCheck ? true }:
    {
      # Automatic package updates
      autoUpdate = if enableAutoUpdate then {
        enable = true;
      ...
      flakeCheck = if enableFlakeCheck then {
        enable = true;
  ...
  mkPlatformPackages = { basePackages ? [], darwinPackages ? [], linuxPackages ? [], aarch64Packages ? [] }:
    let
  ...
  secrets = {
    ...
    getSshSigningKey = 
      let secretInfo = safeGetSecret "ssh_signing_key" defaults.user.signingSshKey;
      in secretInfo.value;
      
    # Get GitHub token with no fallback for security
    getGithubToken = 
      let secretInfo = safeGetSecret "github_token" null;
      in if secretInfo.isFromEnv then secretInfo.value else null;
  ...
  mkGitConfigWithSecrets = extraConfig: 
    let 
      secretsHelpers = (import ./helpers.nix { inherit lib; }).secrets;
      sshKey = secretsHelpers.getSshSigningKey;
    in {
