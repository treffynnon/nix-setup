...
* <kbd>Caps lock</kbd> as <kbd>Esc</kbd> when tapped and <kbd>Ctrl</kbd> when

Run Hammerspoon (use Spotlight with <kbd>Command</kbd> + <kbd>Space</kbd>) and configure it to:

* Run at login
* Enable Accessibility
* (optionally) untick "Show dock icon"

The nix derivation that installs Hammerspoon it is in `./overlays/hammerspoon.nix` and the configuration is in `./home-configs/hamm
` and `./home-configs/hammerspoon`.
